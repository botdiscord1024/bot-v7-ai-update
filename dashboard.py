...
# ══════════════════════════════════════════════════════════
#  WELCOMER PAGE (ONLY WELCOME - NO LEAVE MESSAGES AT ALL)
# ══════════════════════════════════════════════════════════
@app.route('/welcomer')
def welcomer():
    gid = get_gid() or 'default'
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
      ...
      <div>
        <div class="card">
          <div class="card-header"><div><h3>👋 Professional Welcomer</h3><p>Configure automated greetings for new members</p></div></div>
          <div class="card-body">
            <div class="toggle-row">
              <div class="toggle-info"><h4>Enable Welcomer Module</h4><p>Greet new members automatically when they join</p></div>
              <label class="toggle"><input type="checkbox" id="w_enabled" {enabled}> <span class="toggle-slider"></span></label>
            </div>
      ...
      <div>
        <div class="card">
          <div class="card-header"><div><h3>😢 Professional Leave Logger</h3><p>Configure automated leave messages for departing members</p></div></div>
          <div class="card-body">
            <div class="toggle-row">
              <div class="toggle-info"><h4>Enable Leave Module</h4><p>Log or announce when members leave the server</p></div>
    ...
    <div class="btn-save-row"><button type="submit" class="btn btn-primary">Save Welcomer & Leave Configurations</button></div>
    </form>

    <div id="toast_welcome" style="display:none;position:fixed;bottom:24px;right:24px;background:#23a55a;color:#fff;padding:12px 20px;border-radius:6px;font-weight:600;font-size:14px;z-index:9999;">✅ Welcomer & Leave configurations saved successfully!</div>
...
def api_welcomer_save():
    gid = get_gid() or 'default'
    cfg = load('config.json')
    cfg.setdefault(gid, {})['welcomer'] = request.json
    save('config.json', cfg)
    import builtins
    if hasattr(builtins, 'refresh_bot_cache'): 
        builtins.refresh_bot_cache()
