...
def render(route, title, desc, body):
    ...
    <body>
      <div class="sidebar">
        ...
        <a href="/moderation" class="nav-item {% if route=='moderation' %}active{% endif %}">??? Moderation</a>
        ...
        <a href="/smashkarts" class="nav-item {% if route=='smashkarts' %}active{% endif %}">??? Smash Karts</a>
...
def moderation():
    ...
    """
    return render('moderation', '??? Moderation Settings', 'Control automod configurations, blacklisted word definitions, and execution protocols', body)
...
def welcomer():
    ...
    message = w_cfg.get('message', '??????? {user.mention}, ????? ????? ? **{server.name}**! ??')
    embed_title = w_cfg.get('embed_title', '?? ??? ??????????!')
    embed_color = w_cfg.get('embed_color', '#5865f2')
    autorole_roles = w_cfg.get('autorole_roles', '')
    dm_message = w_cfg.get('dm_message', '??????! ??????? ??, ?? ?? ?????????? ??? {server.name}!')
    ...
    leave_message = w_cfg.get('leave_message', '?????????? **{user.name}** ??????? ???????. ????????? **{member_count}** ???????. ??')
    leave_embed_title = w_cfg.get('leave_embed_title', '?? ?????????? ???????')
      ...
      <div class="card-body" style="padding: 20px;">
        <div class="toggle-row" style="margin: 0;">
          <div class="toggle-info">
            <h3 style="margin: 0 0 4px 0; color: #fff;">?? Global Module Status</h3>
...
def smashkarts():
    ...
    for rank, (uid, data) in enumerate(sorted_sk, 1):
        ...
        <div class="lb-row">
            ...
            <div class="lb-val" style="color:#57f287;">{wins} Wins ???</div>
        </div>"""
    if not lb_rows:
        lb_rows = '<div class="lb-empty">No active matches recorded yet.</div>'

    body = f"""<div class="card"><div class="card-header"><h3>??? Competitive Leaderboard</h3></div><div class="card-body">{lb_rows}</div></div>"""
    return render('smashkarts', '??? Smash Karts Statistics', 'Global race metrics and win record compilations', body)
